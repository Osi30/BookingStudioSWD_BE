...
public class IntentDetector {
    ...
    public Mono<IntentType> detectIntent(String message) {
        String prompt = """
            ...
            6. STUDIO_INFO - nếu hỏi về phòng chụp, danh sách studio, loại phòng, tình trạng hoạt động.
            7. GENERAL_CHAT - nếu là lời chào, cảm ơn, hoặc không thuộc nhóm nào trên.
        ...
        return geminiClient.generateResponse(prompt)
                .map(resp -> {
                    String clean = resp.trim().toUpperCase()
                            .replaceAll("[^A-Z_]", ""); // loại bỏ dấu chấm hoặc ký tự lạ
