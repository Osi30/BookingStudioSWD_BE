...
public class AdminPriceTableController {
    private final AdminPriceTableService priceTableService;
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping
    public ResponseEntity<BaseResponse> create(@RequestBody AdminPriceTableRequest req) {
        return ResponseEntity.ok(BaseResponse.builder()
    ...
    public ResponseEntity<BaseResponse> update(@PathVariable String id,
                                               @RequestBody AdminPriceTableRequest req) {
        return ResponseEntity.ok(BaseResponse.builder()
                .code(HttpStatus.OK.value())
