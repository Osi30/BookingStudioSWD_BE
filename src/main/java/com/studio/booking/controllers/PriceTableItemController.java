...
public class PriceTableItemController {
    ...
    @GetMapping("/price")
    public ResponseEntity<BaseResponse> getPriceByTypeAndTime(
            @RequestParam LocalDateTime startTime,
            @RequestParam LocalDateTime endTime,
            @RequestParam String studioTypeId
    ) {
        PriceResultResponse response = itemService
                .getPriceByTypeAndTime(studioTypeId, startTime, endTime);

        return ResponseEntity.ok(BaseResponse.builder()
                .code(HttpStatus.CREATED.value())
                .message("Calculate studio type amount successfully!")
                .data(response)
                .build());
    }
