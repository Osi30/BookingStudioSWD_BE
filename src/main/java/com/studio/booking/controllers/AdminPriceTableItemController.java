...
public class AdminPriceTableItemController {
    private final AdminPriceTableItemService itemService;
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping
    public ResponseEntity<BaseResponse> create(@RequestBody AdminPriceTableItemRequest req) {
        return ResponseEntity.ok(BaseResponse.builder()
    ...
    public ResponseEntity<BaseResponse> update(@PathVariable String id,
                                               @RequestBody AdminPriceTableItemRequest req) {
        return ResponseEntity.ok(BaseResponse.builder()
                .code(HttpStatus.OK.value())
